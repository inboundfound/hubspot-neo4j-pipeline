...
class Neo4jLoader:
    ...
    def load_all(
        ...
        for node_type, node_list in nodes.items():
            if node_list:
                self._load_nodes(node_type, node_list, custom_labels[node_type] if node_type in custom_labels else [])
    ...
    def _load_nodes(self, node_type: str, nodes: List[Dict], custom_labels: List[str] = []):
        """Load nodes of a specific type"""
        self.logger.info(f"Loading {len(nodes)} {node_type} nodes")
        custom_labels_str = f"n:{', n:'.join(custom_labels)}, " if len(custom_labels) > 0 else ""

        with self.driver.session() as session:
            # Process in batches
            for i in tqdm(
                range(0, len(nodes), BATCH_SIZE), desc=f"Loading {node_type}"
            ):
